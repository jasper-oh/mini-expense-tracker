...
// import type { CategoryResponse, CategoryWithTransactionCount } from '#types'
...
//       assert.exists(foodCategory)
//       assert.exists(transportCategory)
//       assert.equal(foodCategory!.transactionCount, 2)
//       assert.equal(transportCategory!.transactionCount, 1)
//     })

//     test('should return categories with zero transaction count when no transactions exist', async ({ assert }) => {
//       // Arrange
//       const testCategories = [
//         { name: 'Food' },
//         { name: 'Transport' },
...
//     test('should order categories by name alphabetically', async ({ assert }) => {
//       // Arrange
//       const testCategories = [
//         { name: 'Zebra' },
//         { name: 'Alpha' },
...
//       const names = result.map(c => c.name)
//       assert.deepEqual(names, ['Alpha', 'Beta', 'Zebra'])
//     })
